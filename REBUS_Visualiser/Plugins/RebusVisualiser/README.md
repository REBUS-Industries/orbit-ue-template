...
> ### Symptoms
>
> Three independent visual regressions on the v1.0.127 build:
>
> 1. **Gobo not projecting in beam.** Operator selects a non-Open gobo via portal `SetFixtureGobo`. The SpotLight's projected light-function cookie on the floor DOES render the gobo pattern correctly (because v1.0.49+ wires the cookie via `SpotLight->LightFunctionMaterial`), but the volumetric `BeamCone` raymarched shaft is a uniform gradient — no gobo silhouette visible inside the beam.
> 2. **Cone wide-base radius ≠ lens radius.** After v1.0.127's orientation flip, the wide ring of the cone (`x=0`, near the lens) is correctly co-located with the SpotLight origin, but its radius is `~3cm` (the pre-v1.0.128 `RebusBeamLensRadiusFloorCm` floor) which is noticeably smaller than the physical lens aperture of the moving-head's mesh body. Visually the cone shaft appears to "shrink into" the fixture body before opening out, instead of starting flush at the lens face.
> 3. **Wrong-side shading on fixture chassis.** On the user's reference screenshot, the middle fixture's chassis has light bleeding through onto faces that should be in shadow. The SpotLight is firing downward; the BOTTOM of the chassis (facing the floor pool, away from the SpotLight body) should read dark; the TOP of the chassis (facing the truss above, on the shadow side of the SpotLight) should read dark. Instead the visible lit-face appears to be the BACK of the mesh's opposite side — classic inverted-normals signature.
>
> ### Root causes
>
> 1. **No beam-side gobo sampler in `M_RebusBeam`.** The on-disk `M_RebusBeam` master (revision 121, pre-v1.0.128) shipped with raymarch parameters for `BeamIntensity`, `BeamColor`, `BeamSharpness`, `BeamFalloff`, `BeamDensity`, `StepCount`, `BeamLength`, `LensRadius`, `FarRadius`, `BeamOrigin`, `BeamDir`, `BeamLightFwd/Right/Up`, `BeamLightFOV`, `BeamShadowMask`, `BeamShadowMaskEnabled` and `BeamMaterialRevision` — but NO `GoboTexture` texture parameter and NO `GoboEnable` scalar. The `_BEAM_RAYMARCH_HLSL` body in `build_rebus_base_level.py::_build_beam_master` had no gobo sampling code path. The Epic-beam `M_LightBeam` path (the alternate beam shaft used when `Rebus.PreferProceduralBeam 0`) does have a `Gobo` texture parameter that the v1.0.49+ `ApplyCurrentGoboToEpicBeam` writes correctly, but the procedural `BeamCone` `M_RebusBeam` path (the default, used in `Rebus.PreferProceduralBeam 1` mode) was missing the equivalent sampler entirely. A `SetTextureParameterValue(TEXT("GoboTexture"), ...)` against a parameter the master doesn't declare is a benign no-op on UE 5.7's `UMaterialInstanceDynamic` — silently swallows the call and the visual stays unchanged.
> 2. **Lens-radius floor too small AND no per-fixture override.** `BeamBaseRadiusUnreal` was being seeded as `max(photometrics.LensDiameter/2, RebusBeamLensRadiusFloorCm)` where the floor was `3.0f` (centimetres). On most portal-fed fixtures `photometrics.LensDiameter` is populated correctly and the floor never engages — but the user's session had at least one fixture where the portal sent a `LensDiameter` that was either missing OR genuinely smaller than the visible lens face on the mesh (LD pipelines disagree about whether "lens diameter" means the throw aperture or the front-element diameter; GDTF doesn't disambiguate). With the floor at 3cm the cone shaft's wide-base ring was visibly smaller than the rendered lens face on the fixture's chassis mesh. There was no per-fixture operator override to bypass the portal value.
> 3. **`PointYUpMetersToUnreal` flips handedness; triangle winding never compensated.** `Public/RebusCoordinates.h:47`'s comment says "Y-up metres -> Unreal Z-up centimetres" and the body maps `(x, y, z) -> (x, z, y) * 100`. This is a Y<->Z basis swap — an ODD permutation with `det(transform) = -1` — which flips handedness from the source's right-handed system to Unreal's left-handed system. Under a handedness flip, CCW-wound triangles in source space become CW-wound in target space and read as back-facing under the LH front-face convention; the per-face normal computed downstream via `cross(B-A, C-A)` ALSO flips sign (cross product is a pseudovector that inverts under handedness change), so the per-vertex smoothed normals point INWARD rather than outward. The result is that even with a two-sided material the lighting computation reads the wrong-direction normal and lights the back face. This has been latent since the v1.0.0 mesh import path but was MASKED on every fixture with a `bDriveOrbitModel=true` Orbit binding (the v1.0.65 default), because the Orbit-imported overlay mesh comes via glTFRuntime's correctly-oriented import path and overdraws the broken control-channel proxy.
>
> ### Code fixes (precise diffs)
>
> 1. **`Content/Python/build_rebus_base_level.py::_build_beam_master`** — add `GoboTexture` (TextureObjectParameter, default `T_RebusGoboDefault.T_RebusGoboDefault`) and `GoboEnable` (scalar, default 0.0) to the master, and extend `_BEAM_RAYMARCH_HLSL` to sample the gobo planar-projected from the apex (perpendicular offset from the cone axis, normalized by `radiusAt`, scaled into UV space, sampled via `Texture2DSample(GoboTexture, GoboTextureSampler, goboUV)`, modulating `BeamDensity` by the sample's luma when `GoboEnable > 0`). The `[branch]` keyword on the `GoboEnable > 0` test keeps the per-step cost zero when no gobo is selected. Bumped `REBUS_BEAM_MATERIAL_REVISION` from `121` to `122`. Extended `_beam_master_has_shadow_mask` (despite the legacy name, this is the master-feature probe) to also require `GoboEnable` + `GoboTexture` so a stale on-disk master with revision >= 121 but missing the gobo params still triggers self-heal.
> 2. **`Source/RebusVisualiser/Private/RebusFixtureActor.cpp`** — added `void ARebusFixtureActor::RefreshBeamGoboParams()` (`~70` lines) that resolves the LIVE cone slot-0 MID via `Cast<UMaterialInstanceDynamic>(BeamCone->GetMaterial(0))` AND falls back to the cached `BeamMID` UPROPERTY, then pushes `GoboTexture = GoboRT` (the per-fixture `UCanvasRenderTarget2D` cookie) + `GoboEnable = (bGoboActive && GoboRT != nullptr) ? 1.0f : 0.0f` onto BOTH iff distinct (v1.0.125 double-push pattern). Wired into THREE call sites: tail of `BuildBeamCone` (seed for fresh-spawn fixtures), tail of `ApplyCurrentGoboToEpicBeam` (so `SetFixtureGobo` push flows through the cone-side path in lockstep with the Epic-beam-side path), and inside `OnGoboRTUpdate` (so the per-frame canvas redraw kicks a re-push and the cone never reads a stale cookie). New `BeamStartRadius` UPROPERTY (`UPROPERTY(EditAnywhere, Category="Rebus|Beam", meta=(ClampMin="0.5", UIMin="0.5", UIMax="30.0", Units="Centimeters")) float BeamStartRadius = 5.0f;`) declared in `Public/RebusFixtureActor.h`. `BuildBeamCone` updated to seed `BeamBaseRadiusUnreal` as `max(photometrics.LensDiameter/2, BeamStartRadius, RebusBeamLensRadiusFloorCm)` so the operator's per-fixture choice cannot be silently shrunk by a too-small portal value, and the constant `RebusBeamLensRadiusFloorCm` was raised from `3.0f` to `5.0f` so the second-level safety floor matches the new UPROPERTY default. `BuildMeshes` triangle-emit loop swapped `Ia` <-> `Ib` in the fan-triangulation (`Triangles.Add(I0); Triangles.Add(Ib); Triangles.Add(Ia);` was `Triangles.Add(I0); Triangles.Add(Ia); Triangles.Add(Ib);`) so the winding flip compensates for the `PointYUpMetersToUnreal` handedness inversion; the cross-product normal computation below now produces outward-pointing normals. `DumpBeamDiagForDebug` extended with `Gobo={bGoboActive=%d goboIndex=%d goboWheel=%d srcTex=%s(%p) goboRT=%p}` and `MIDReadback.GoboEnable=%s` so `Rebus.DiagBeam` surfaces the gobo state alongside `BeamIntensity` for one-paste triage. All version-tag log strings bumped `v1.0.127` -> `v1.0.128` in `BuildBeamCone` SPAWNED line, `LogBeamCone` dump banner, and both `DumpBeamDiagForDebug` banners; SPAWNED line gains three new fields `beamStartRadius=%.2fcm goboPlumbing=v1.0.128 windingFlipped=v1.0.128` so the operator can confirm from log scan which v1.0.128 fix landed on which fixture.
> 3. **`Source/RebusVisualiser/Private/RebusVisualiserSubsystem.cpp`** — `RebusExpectedBeamMaterialRevision` bumped `121` -> `122`. Self-healing `SelfHealBeamMaterialRevisionIfMismatched` now triggers an auto-purge + re-bake warning on every fixture whose on-disk master is < 122, so the operator gets a LOUD diagnostic if they reload the v1.0.128 DLL against the v1.0.127 baked master. Startup-banner triage text `if pluginVersion != v1.0.127` -> `v1.0.128`.
> 4. **`Source/RebusVisualiser/Private/RebusVisualiser.cpp`** — `Rebus.DiagBeam` begin/end banner version strings `v1.0.127` -> `v1.0.128`.
> 5. **`RebusVisualiser.uplugin`** — `VersionName` `1.0.127` -> `1.0.128`.
> 6. **`README.md`** — this v1.0.128 release block.
>
> ### `M_RebusBeam` rebake required (YES — revision 121 -> 122)
>
> v1.0.128 Issue 1 added two new master-material parameters (`GoboTexture`, `GoboEnable`) AND new HLSL inside the raymarch loop, so the on-disk `M_RebusBeam.uasset` MUST be re-baked for the gobo to project inside the volumetric beam shaft. The C++ self-heal probe at `BuildBeamCone` tail will detect a stale master (revision < 122) and log a Warning per fixture, but the master cannot be regenerated from a `-game` mode session (editor-only Python APIs are not available — `EditorAssetLibrary`, `MaterialEditingLibrary` and `AssetToolsHelpers` all live in `EditorScriptingUtilities.dll` which is editor-only since UE 5.7). Operator MUST regenerate the master via ONE of:
>
> ```
> # Option A (interactive editor session):
> #   Tools > Execute Python Script > REBUS_Visualiser/Content/Python/build_rebus_base_level.py
> # Option B (headless commandlet -- v1.0.121 path):
> "$UE_ROOT/Engine/Binaries/Win64/UnrealEditor-Cmd.exe" \
>     "$REPO/REBUS_Visualiser/REBUS_Visualiser.uproject" \
>     -run=pythonscript \
>     -script="$REPO/REBUS_Visualiser/Content/Python/build_rebus_base_level.py"
> ```
>
> Until the rebake lands the `Rebus.DiagBeam` dump will read `MIDReadback.MatRevision=121` (the old master), `MIDReadback.GoboEnable=MISSING` (the parameter doesn't exist on the parent), and `Gobo.bGoboActive=1 goboRT=0x...` (the C++ push fired but had nowhere to land). After the rebake those read `MatRevision=122`, `GoboEnable=1.00`, and the gobo silhouette appears inside the beam.
>
> Issues 2 and 3 are PURE C++ fixes — no master-material change. They take effect on DLL reload alone, no rebake needed.
>
> ### User action required (in order)
>
> 1. **Pull the v1.0.128-ue5.7 tag + reload the editor / `-game` session.** The C++ DLL reload picks up Issue 2 (cone base radius) and Issue 3 (chassis winding fix) immediately. Verify: the SPAWNED log line for any fixture reads `SPAWNED v1.0.128 ... beamStartRadius=5.00cm goboPlumbing=v1.0.128 windingFlipped=v1.0.128`; the middle un-Orbit-bound fixture's chassis now lights the correct face under the SpotLight.
> 2. **Run the M_RebusBeam rebake.** Use Option A (interactive editor) OR Option B (headless commandlet) from the rebake section above. Verify: log line `LogRebusVisualiser: [Rebus] M_RebusBeam regenerated -- revision 121 -> 122 (gobo plumbing)`. Save the `.uasset` (Option A) or commit the regenerated `M_RebusBeam.uasset` (Option B).
> 3. **Reload the level (or `r.ReloadMaterials`) so the new master binds.** Verify in `Rebus.DiagBeam`: `MIDReadback.MatRevision=122`, `MIDReadback.GoboEnable=1.00` for any fixture with a non-Open gobo selected, AND the `Gobo` line shows `bGoboActive=1 goboRT=0x<non-null>`. Visually: the gobo silhouette is now visible inside the volumetric beam shaft, matching the floor cookie projection.
> 4. **Set `BeamStartRadius` per-fixture in the editor (optional override).** Default 5.0cm matches median moving-head lens radius and should be visually correct for most fixtures. For hero fixtures with notably-different lens apertures, set `BeamStartRadius` in the `Rebus|Beam` category on `ARebusFixtureActor` (clamp [0.5..30.0] cm). The cone wide-base ring will rebuild on the next `BuildBeamCone` call (on level reload, or trigger via `Rebus.Force RebuildBeam <fixtureId>` if that hook exists in your build).
